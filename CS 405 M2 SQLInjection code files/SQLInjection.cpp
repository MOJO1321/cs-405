...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  ...
  //  something more generic
    ...
    std::regex GetSQLInjection("[^0-9A-Z+$]\\s*(and|or)\\s*[\\'\"]{0,1}", std::regex_constants::icase);
    if (std::regex_search(sql, GetSQLInjection))
    {
        std::string warning = std::string("A SQL INJECTION HAS BEEN DISCOVERED! INVALID USER ATTEMPTING ACCESS! TAKE IMMEDIATE ACTION!");
        std::cout << "WARNING WARNING FAILURE! ERROR = " << warning << std::endl;
        return false;
    }
